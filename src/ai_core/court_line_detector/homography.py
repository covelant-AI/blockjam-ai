        ...
        if not any([None in x for x in inters]):
            ...
            for i in range(12):
                if i not in inds and points[i][0] is not None:
                    # Convert points[i] to 1-D numpy array and flatten trans_kps[i]
                    point_array = np.array(points[i], dtype=np.float32).flatten()
                    trans_point = trans_kps[i].flatten()
                    dists.append(distance.euclidean(point_array, trans_point))
